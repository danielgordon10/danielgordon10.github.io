$(document).ready(function () {
  ...
  var lastPathSegment = href.substr(
    href.lastIndexOf("/") + 1,
    href.indexOf(".html"),
  );
  if (lastPathSegment == "index.html" || lastPathSegment == "") {
    jsonFile = "featured_projects.json";
    basePath = "";
    dataPath = "";
  } else {
    jsonFile = "zz_file_order.json";
    basePath =
      lastPathSegment.substring(0, lastPathSegment.indexOf(".html")) + "/";
    dataPath = basePath + "info/";
  }

  $.getJSON(dataPath + jsonFile, function (orderData) {
    var projectsContainer = $("#projects-container");
    orderData.forEach(function (currVal, index) {
      projectsContainer.append('<div id="outerContainer' + index + '"></div>');
    });
    orderData.forEach(function (currVal, index) {
      $.getJSON(dataPath + currVal, function (data) {
        ...
        if (data.vid_id.length > 0) {
          element.append(
            '<div class="bg-gray py-2" id="outer' +
              index +
              '">' +
              ...
              "</div>" +
              "</div>" +
              '<div class="col-md-6 px-3" onclick="location.href=\'' +
              data.url +
              '\';" style="cursor: pointer;">' +
              '<div class="row">' +
              '<div class="col-md-12">' +
              '<h3 class="text-dark padding-top-1">' +
              data.title +
              "</h3>" +
              "<hr>" +
              "</div>" +
          ...
          element.append(
            '<div class="bg-gray py-2" id="outer' +
              ...
              '<div class="row shadow-sm">' +
              '<div class="p-0 col-md-6">' +
              '<img class="img-fluid d-block" src="images/projects/' +
              data.img_path +
              '">' +
              "</div>" +
              '<div class="col-md-6 px-3" onclick="location.href=\'' +
