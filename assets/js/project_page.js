$(document).ready(function () {
  ...
  var lastPathSegment =
    href.substring(href.lastIndexOf("/") + 1, href.indexOf(".html")) + ".json";
  $.getJSON("info/" + lastPathSegment, function (data) {
    $("#page-title").html(data.title);
    if (
      data.hasOwnProperty("title_project_page") &&
      data.title_project_page.length > 0
    ) {
    ...
    if (data.vid_id.length > 0) {
      $("#vid-frame").attr(
        "src",
        "https://www.youtube.com/embed/" + data.vid_id + "?autoplay=0",
      );
    ...
    data.buttons.forEach(function (currVal, index) {
      var buttonElement = $(
        "<a class='btn btn-lg mx-2 text-uppercase btn-info project-page-btn', target='_blank'></a>",
      );
